...
public class ConfigParser {
	...
	private ArcherConfig archerConfig;
	private EngineerConfig engineerConfig;
	...
	public FinaleManager parse() {
		...
		archerConfig = new ArcherConfig(config.getConfigurationSection("archer"));
		engineerConfig = new EngineerConfig(config.getConfigurationSection("engineer"));
		...
		manager = new FinaleManager(debug, attackEnabled, attackSpeed, invulnerableTicks, regenEnabled, regenhandler, weapMod, armourMod,
				potionHandler, combatConfig, archerConfig, engineerConfig);
	...
	private ArmourModifier parseArmourModification(ConfigurationSection config) {
		...
		for (String key : config.getKeys(false)) {
			...
			int toughness = current.getInt("toughness", -1);
			int armour = current.getInt("armour", -1);
	...
	private CombatConfig parseCombatConfig(ConfigurationSection config) {
		...
		return new CombatConfig(noCooldown, cpsLimit, cpsCounterInterval, maxReach, sweepEnabled, combatSounds, horizontalKb, verticalKb,
				sprintHorizontal, sprintVertical, airHorizontal, airVertical, waterHorizontal, waterHorizontal, attackMotionModifier, stopSprinting, potionCutOffDistance);
	}
